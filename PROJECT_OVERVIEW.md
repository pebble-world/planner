...
| D12 | 🟡 | Misc smells: `_paintHandle` uses redundant `color.withAlpha(255)`; labels use hardcoded offsets (15/60) and aren't centered; `verticalScroll` uses a fixed 20px step ignoring zoom; O(n) linear hit-testing; **hardcoded English** menu strings (no l10n); zoom buttons/colors not configurable; legacy APIs (`RawMaterialButton`, `primarySwatch`, old `typedef` syntax); `PlannerEntry`/`PlannerTime` are mutable with no `==`/`copyWith`; **no `Semantics`/accessibility** on the canvas. | Address opportunistically during the refactor. |
